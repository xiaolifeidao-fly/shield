...
const READ_NAVIGATOR_PLATFORM_SCRIPT = String.raw`function () {
    const navigatorObj = navigator;
    const result = {};
    for (const key in navigatorObj) {
...
function resolveEnginePath(configuredPath: string | undefined, fallbackSegments: string[]): string {
  if (configuredPath) {
    return path.isAbsolute(configuredPath)
      ? configuredPath
      : path.resolve(process.cwd(), configuredPath);
  }
  return path.join(getEngineRootPath(), ...fallbackSegments);
}

// Fallback for non-Electron environment (Node.js)
function getEngineRootPath(): string {
  const configuredRoot = process.env.USER_DATA_PATH || process.env.SHIELD_ENGINE_ROOT_DIR;
  if (configuredRoot) {
  ...
  if (electronApp && typeof electronApp.getPath === 'function' && electronApp.isReady?.()) {
    return path.join(electronApp.getPath('userData'), 'shield-engine');
  }

  return path.resolve(process.cwd(), '.runtime', 'shield-engine');
}

function getSessionBasePath(): string {
  return resolveEnginePath(process.env.SHIELD_ENGINE_SESSION_DIR, ['resource', 'session']);
}

function getUserDataBasePath(): string {
  return resolveEnginePath(process.env.SHIELD_ENGINE_USER_DATA_DIR, ['resource', 'userDataDir']);
}
...
function shouldDisableChromiumSandbox(): boolean {
    ...
    return os.platform() === 'linux' && typeof process.getuid === 'function' && process.getuid() === 0;
}
...
function getChromiumLaunchOptions() {
    ...
    const headless = shouldForceHeadless();
...
export abstract class DoorEngine<T = any> {
    ...
    async addAntiDetectionScript(context: BrowserContext) {
        await context.addInitScript(() => {
            ...
            // 0. 删除 Angular/Zone.js 的 __name 全局变量，避免 page.evaluate 冲突
            try {
                // @ts-ignore
...
export async function setPlatform(page : Page){
    const platform = await evaluateSerializedScript(page, READ_NAVIGATOR_PLATFORM_SCRIPT);
...
export async function getPlatform(){
    ...
    if(browserPlatform){
        // mysqlCache already deserializes JSON, so browserPlatform might be an object
        if (typeof browserPlatform === 'string') {

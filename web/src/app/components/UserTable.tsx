...
interface UserTableProps {
  ...
  onRun: (username: string, businessType: string) => void | Promise<void>;
...
const UserTable: React.FC<UserTableProps> = ({
  ...
  const handleRun = (username: string, userId: string, businessType: string) => {
    // 展开对应的行
              ...
              <span
                onClick={() => {
                  if (record.businessType && !isRunning && !runningUsers.has(record.username)) {
                    handleRun(record.username, record.id, record.businessType);
                  }
                }}
                style={{ 
